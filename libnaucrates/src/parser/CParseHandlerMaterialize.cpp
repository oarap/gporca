...
//
//---------------------------------------------------------------------------
CParseHandlerMaterialize::CParseHandlerMaterialize(IMemoryPool *memory_pool,
...
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalMaterialize),
									  element_local_name))
	{
		GPOS_ASSERT(this->Length() == 0 && "No handlers should have been added yet");

		m_dxl_op = (CDXLPhysicalMaterialize *) CDXLOperatorFactory::MakeDXLMaterialize(
