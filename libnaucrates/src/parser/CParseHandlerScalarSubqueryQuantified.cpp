...
//---------------------------------------------------------------------------
void
CParseHandlerScalarSubqueryQuantified::StartElement(const XMLCh *const,  // element_uri,
													...
													const Attributes &attrs)
{
	...
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarSubqueryAny),
									  element_local_name))
	{
		dxl_token = EdxltokenScalarSubqueryAny;
	}
	else if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarSubqueryAll),
										   element_local_name))
	{
	...
	IMDId *mdid_op = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenOpNo, dxl_token);

	// parse operator name
	const XMLCh *xmlszScalarOpName =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenOpName, dxl_token);
	...
	{
		m_dxl_op = GPOS_NEW(m_memory_pool)
			CDXLScalarSubqueryAny(m_memory_pool, mdid_op, md_op_name, col_id);
	}
	else
	{
		m_dxl_op = GPOS_NEW(m_memory_pool)
			CDXLScalarSubqueryAll(m_memory_pool, mdid_op, md_op_name, col_id);
	}

	// parse handler for the child nodes
	CParseHandlerBase *parse_handler_logical_child = CParseHandlerFactory::GetParseHandler(
	...
	CParseHandlerBase *parse_handler_scalar_child = CParseHandlerFactory::GetParseHandler(
...
//---------------------------------------------------------------------------
void
CParseHandlerScalarSubqueryQuantified::EndElement(const XMLCh *const,  // element_uri,
												  const XMLCh *const element_local_name,
												  const XMLCh *const  // element_qname
)
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarSubqueryAll),
									  element_local_name) &&
		0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarSubqueryAny),
									  element_local_name))
	{
		CWStringDynamic *str = CDXLUtils::CreateDynamicStringFromXMLChArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), element_local_name);
