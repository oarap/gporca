...
//
//---------------------------------------------------------------------------
CParseHandlerCondList::CParseHandlerCondList(IMemoryPool *memory_pool,
...
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarHashCondList),
									  element_local_name))
	{
		// start the hash cond list
		m_dxl_node = GPOS_NEW(m_memory_pool)
			CDXLNode(m_memory_pool, GPOS_NEW(m_memory_pool) CDXLScalarHashCondList(m_memory_pool));
	}
	else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarMergeCondList),
										   element_local_name))
	{
		// start the merge cond list
		m_dxl_node = GPOS_NEW(m_memory_pool)
			CDXLNode(m_memory_pool, GPOS_NEW(m_memory_pool) CDXLScalarMergeCondList(m_memory_pool));
	}
	else
	{
...
)
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarHashCondList),
									  element_local_name) &&
		0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarMergeCondList),
									  element_local_name))
	{
