...
//---------------------------------------------------------------------------
void
CParseHandlerDynamicTableScan::StartElement(const XMLCh *const,  // element_uri,
...
)
{
	...
	table_descr->AddRef();
	CDXLPhysicalDynamicTableScan *dxl_op = GPOS_NEW(m_memory_pool) CDXLPhysicalDynamicTableScan(
		m_memory_pool, table_descr, m_part_index_id, m_part_index_id_printable);
