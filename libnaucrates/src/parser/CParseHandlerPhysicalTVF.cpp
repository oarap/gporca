...
//
//---------------------------------------------------------------------------
CParseHandlerPhysicalTVF::CParseHandlerPhysicalTVF(IMemoryPool *memory_pool,
...
{
	if (0 ==
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalTVF), element_local_name))
	{
		// parse function id
		...
		const XMLCh *xml_str_func_name =
			CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenPhysicalTVF);
...
)
{
	...
	CDXLPhysicalTVF *dxl_op = GPOS_NEW(m_memory_pool)
		CDXLPhysicalTVF(m_memory_pool, m_func_mdid, m_return_type_mdid, m_pstr);
