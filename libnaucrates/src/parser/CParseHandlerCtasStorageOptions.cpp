...
//---------------------------------------------------------------------------
void
CParseHandlerCtasStorageOptions::StartElement(const XMLCh *const,  // element_uri
											  const XMLCh *const element_local_name,
											  const XMLCh *const,  // element_qname
											  const Attributes &attrs)
{
	if (0 ==
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenCTASOptions), element_local_name))
	{
										   ...
										   element_local_name))
	{
		...
		ULONG ctas_option_type = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
			...
			EdxltokenCTASOption);
		CWStringBase *ctas_option_name = CDXLOperatorFactory::ExtractConvertAttrValueToStr(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenName, EdxltokenCTASOption);
		CWStringBase *ctas_option_val = CDXLOperatorFactory::ExtractConvertAttrValueToStr(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenValue, EdxltokenCTASOption);
		...
		if (NULL == m_ctas_storage_option_array)
		{
			m_ctas_storage_option_array =
				GPOS_NEW(m_memory_pool) CDXLCtasStorageOptions::DXLCtasOptionArray(m_memory_pool);
		}
		m_ctas_storage_option_array->Append(
			GPOS_NEW(m_memory_pool) CDXLCtasStorageOptions::CDXLCtasOption(
...
//---------------------------------------------------------------------------
void
CParseHandlerCtasStorageOptions::EndElement(const XMLCh *const,  // element_uri,
											const XMLCh *const element_local_name,
											const XMLCh *const  // element_qname
)
{
	if (0 ==
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenCTASOptions), element_local_name))
	{
		m_dxl_ctas_storage_option = GPOS_NEW(m_memory_pool) CDXLCtasStorageOptions(
			m_mdname_tablespace, m_ctas_on_commit_action, m_ctas_storage_option_array);
		// deactivate handler
