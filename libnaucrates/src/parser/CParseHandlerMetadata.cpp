...
{
	if (0 ==
		XMLString::compareString(element_local_name, CDXLTokens::XmlstrToken(EdxltokenMetadata)))
	{
		...
		m_system_id_array = GetSrcSysIdArray(attrs, EdxltokenSysids, EdxltokenMetadata);
			 ...
			 XMLString::compareString(element_local_name, CDXLTokens::XmlstrToken(EdxltokenMdid)))
	{
		// start of the metadata section in the DXL document
										...
										Edxltoken dxl_token_element)
{
	...
	while (NULL != (xml_str_sys_id = xml_str_tokenizer.nextToken()))
	{
		...
		XMLStringTokenizer xml_str_sys_id_tokenizer(xml_str_sys_id,
