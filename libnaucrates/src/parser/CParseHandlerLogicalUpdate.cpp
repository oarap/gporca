...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalUpdate::CParseHandlerLogicalUpdate(IMemoryPool *memory_pool,
...
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalUpdate),
									  element_local_name))
	{
	...
	const XMLCh *delete_colids_xml =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenDeleteCols, EdxltokenLogicalUpdate);
	...
	const XMLCh *insert_colids_xml =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenInsertCols, EdxltokenLogicalUpdate);
...
)
{
	...
	m_dxl_node = GPOS_NEW(m_memory_pool)
		CDXLNode(m_memory_pool,
				 GPOS_NEW(m_memory_pool) CDXLLogicalUpdate(m_memory_pool,
														   table_descr,
														   m_ctid_colid,
														   m_segid_colid,
														   m_deletion_colid_array,
