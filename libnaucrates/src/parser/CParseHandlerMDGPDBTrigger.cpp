...
{
	if (0 !=
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBTrigger), element_local_name))
	{
	...
	m_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBTrigger);

	const XMLCh *xml_str_name =
	...
	m_func_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenFuncId, EdxltokenGPDBTrigger);
