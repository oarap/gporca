...
{
	if (0 ==
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBAgg), element_local_name))
	{
		// parse agg name
		const XMLCh *xml_str_agg_name =
			CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenGPDBAgg);

		CWStringDynamic *str_agg_name = CDXLUtils::CreateDynamicStringFromXMLChArray(
		...
		m_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBAgg);
		...
		const XMLCh *xml_str_hash_agg_capable =
			attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenGPDBAggHashAggCapable));
	...
	else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBAggResultTypeId),
										   element_local_name))
	{
		// parse result type
		...
		m_mdid_type_result = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
			...
			EdxltokenGPDBAggResultTypeId);
					  ...
					  CDXLTokens::XmlstrToken(EdxltokenGPDBAggIntermediateResultTypeId),
					  element_local_name))
	{
