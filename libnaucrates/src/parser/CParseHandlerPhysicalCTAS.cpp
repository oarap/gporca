...
//
//---------------------------------------------------------------------------
CParseHandlerPhysicalCTAS::CParseHandlerPhysicalCTAS(IMemoryPool *mp,
...
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalCTAS),
									  element_local_name))
	{
	...
	// parse table name
	const XMLCh *table_name_xml =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenPhysicalCTAS);
	...
	const XMLCh *src_colids_xml =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenInsertCols, EdxltokenPhysicalCTAS);
	m_src_colids_array =
		CDXLOperatorFactory::ExtractIntsToUlongArray(m_parse_handler_mgr->GetDXLMemoryManager(),
													 src_colids_xml,
													 EdxltokenInsertCols,
...
)
{
	...
	m_dxlnode = GPOS_NEW(m_mp)
		CDXLNode(m_mp,
				 GPOS_NEW(m_mp) CDXLPhysicalCTAS(m_mp,
														  m_mdname_schema,
														  m_mdname,
														  dxl_col_descr_array,
														  ctas_options,
