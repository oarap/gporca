								  ...
								  const Attributes &attrs)
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMDType), element_local_name))
	{
		...
		m_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenMDType);

		if (!IsBuiltInType(m_mdid))
		{
			// parse type name
			const XMLCh *xmlszTypeName =
				CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenMDType);

			CWStringDynamic *pstrTypeName = CDXLUtils::CreateDynamicStringFromXMLChArray(
			...
			const XMLCh *attribute_val_xml =
				attrs.getValue(CDXLTokens::XmlstrToken(EdxltokenMDTypeComposite));
...
//---------------------------------------------------------------------------
void
CParseHandlerMDType::ParseMdid(const XMLCh *element_local_name, const Attributes &attrs)
{
	...
	for (ULONG ul = 0; ul < GPOS_ARRAY_SIZE(rgmdidmap); ul++)
	{
		...
		if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(mdidmapelem.m_edxltoken),
										  element_local_name))
		{
	...
	*token_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, token_type);
}
