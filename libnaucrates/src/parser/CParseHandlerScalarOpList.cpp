...
//
//---------------------------------------------------------------------------
CParseHandlerScalarOpList::CParseHandlerScalarOpList(IMemoryPool *memory_pool,
	...
	if (NULL == m_dxl_node && CDXLScalarOpList::EdxloplistSentinel > dxl_op_list_type)
	{
		...
		m_dxl_node = GPOS_NEW(m_memory_pool)
			CDXLNode(m_memory_pool,
					 GPOS_NEW(m_memory_pool) CDXLScalarOpList(m_memory_pool, m_dxl_op_list_type));
	}
	else
	{
...
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarOpList),
									  element_local_name))
	{
		return CDXLScalarOpList::EdxloplistGeneral;
	}

	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPartLevelEqFilterList),
									  element_local_name))
	{
		return CDXLScalarOpList::EdxloplistEqFilterList;
	}

	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPartLevelFilterList),
									  element_local_name))
	{
