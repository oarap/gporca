...
//
//---------------------------------------------------------------------------
CParseHandlerScalarNullIf::CParseHandlerScalarNullIf(IMemoryPool *memory_pool,
...
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarNullIf),
									  element_local_name))
	{
	...
	IMDId *mdid_op = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenOpNo, EdxltokenScalarNullIf);
	...
	m_dxl_node = GPOS_NEW(m_memory_pool) CDXLNode(
		m_memory_pool,
		GPOS_NEW(m_memory_pool) CDXLScalarNullIf(m_memory_pool, mdid_op, mdid_return_type));
