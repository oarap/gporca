...
//
//---------------------------------------------------------------------------
CParseHandlerMDRelation::CParseHandlerMDRelation(IMemoryPool *memory_pool,
...
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPartConstraint),
									  element_local_name))
	{
		...
		{
			m_level_with_default_part_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
	...
	if (0 !=
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenRelation), element_local_name))
	{
	...
	// parse storage type
	const XMLCh *xmlszStorageType =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenRelStorageType, EdxltokenRelation);
	...
	{
		m_num_of_partitions =
			CDXLOperatorFactory::ConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(),
														 xmlszPartitions,
...
)
{
									  ...
									  element_local_name))
	{
		...
		else
		{
			...
			m_part_constraint = GPOS_NEW(m_memory_pool) CMDPartConstraintGPDB(
				m_memory_pool, m_level_with_default_part_array, m_part_constraint_unbounded, NULL);
		}
		return;
	}
	...
	m_imd_obj = GPOS_NEW(m_memory_pool) CMDRelationGPDB(m_memory_pool,
														m_mdid,
														m_mdname,
														m_is_temp_table,
														m_rel_storage_type,
												 ...
												 Edxltoken dxl_token_element)
{
	...
	const XMLCh *xml_str_table_name =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, dxl_token_element);
	...
	m_mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, dxl_token_element);

	// parse distribution policy
	const XMLCh *rel_distr_policy_xml =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenRelDistrPolicy, dxl_token_element);
	...
	if (m_rel_distr_policy == IMDRelation::EreldistrHash)
	{
		...
		const XMLCh *rel_distr_cols_xml =
			CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenDistrColumns, dxl_token_element);
		m_distr_col_array =
			CDXLOperatorFactory::ExtractIntsToUlongArray(m_parse_handler_mgr->GetDXLMemoryManager(),
														 rel_distr_cols_xml,
...
CParseHandlerMDRelation::ParseChildNodes()
{
	...
	CParseHandlerBase *check_constraint_list_parse_handler = CParseHandlerFactory::GetParseHandler(
	...
	CParseHandlerBase *trigger_list_parse_handler = CParseHandlerFactory::GetParseHandler(
		m_memory_pool, CDXLTokens::XmlstrToken(EdxltokenMetadataIdList), m_parse_handler_mgr, this);
	m_parse_handler_mgr->ActivateParseHandler(trigger_list_parse_handler);

	// parse handler for index info list
	CParseHandlerBase *index_info_list_parse_handler = CParseHandlerFactory::GetParseHandler(
		m_memory_pool, CDXLTokens::XmlstrToken(EdxltokenIndexInfoList), m_parse_handler_mgr, this);
