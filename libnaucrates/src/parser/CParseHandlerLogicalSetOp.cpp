										...
										const Attributes &attrs)
{
	if (0 == this->Length())
	{
		...
		m_input_colids_arrays = CDXLOperatorFactory::ExtractConvertUlongTo2DArray(
...
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalUnion),
									  element_local_name))
	{
		return EdxlsetopUnion;
	}

	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalUnionAll),
									  element_local_name))
	{
		return EdxlsetopUnionAll;
	}

	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalIntersect),
									  element_local_name))
	{
		return EdxlsetopIntersect;
	}

	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalIntersectAll),
									  element_local_name))
	{
		return EdxlsetopIntersectAll;
	}

	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalDifference),
									  element_local_name))
	{
		return EdxlsetopDifference;
	}

	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalDifferenceAll),
									  element_local_name))
	{
...
)
{
	...
	ColumnDescrDXLArray *cold_descr_dxl_array = col_descr_parse_handler->GetColumnDescrDXLArray();
