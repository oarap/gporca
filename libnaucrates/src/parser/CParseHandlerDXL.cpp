...
//
//---------------------------------------------------------------------------
CParseHandlerDXL::CParseHandlerDXL(IMemoryPool *memory_pool,
...
CParseHandlerDXL::IsValidStartElement(const XMLCh *const element_name)
{
	...
	for (ULONG idx = 0; !is_valid_start_element && idx < GPOS_ARRAY_SIZE(valid_start_element_array);
		 idx++)
	{
		is_valid_start_element =
			(0 == XMLString::compareString(element_name, valid_start_element_array[idx]));
	}
							   ...
							   const Attributes &attrs)
{
	...
	if (0 == XMLString::compareString(element_local_name,
									  CDXLTokens::XmlstrToken(EdxltokenDXLMessage)) ||
		0 == XMLString::compareString(element_local_name,
									  CDXLTokens::XmlstrToken(EdxltokenThread)) ||
		0 ==
			XMLString::compareString(element_local_name, CDXLTokens::XmlstrToken(EdxltokenComment)))
	{
...
CParseHandlerDXL::ExtractMetadataObjects(CParseHandlerBase *parse_handler_base)
{
	...
	GPOS_ASSERT(NULL != parse_handler_md && NULL != parse_handler_md->GetMdIdCachedObjArray());
...
CParseHandlerDXL::ExtractMDRequest(CParseHandlerBase *parse_handler_base)
{
	...
	m_mdrequest =
		GPOS_NEW(m_memory_pool) CMDRequest(m_memory_pool, mdid_array, md_type_request_array);
}
