...
{
	if (0 !=
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBMDScCmp), element_local_name))
	{
	...
	const XMLCh *xml_str_op_name =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenGPDBMDScCmp);
	...
	IMDId *mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMdid, EdxltokenGPDBMDScCmp);
	...
	IMDId *mdid_op = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenOpNo, EdxltokenGPDBMDScCmp);
