...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalDelete::CParseHandlerLogicalDelete(IMemoryPool *memory_pool,
...
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalDelete),
									  element_local_name))
	{
	...
	const XMLCh *deletion_colids =
		CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenDeleteCols, EdxltokenLogicalDelete);
...
)
{
	...
	m_dxl_node = GPOS_NEW(m_memory_pool) CDXLNode(
		m_memory_pool,
		GPOS_NEW(m_memory_pool) CDXLLogicalDelete(
			m_memory_pool, table_descr, m_ctid_colid, m_segid_colid, m_deletion_colid_array));
