...
{
	if (0 ==
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenLogicalTVF), element_local_name))
	{
		// parse function id
		...
		// parse function name
		const XMLCh *func_name =
			CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenName, EdxltokenLogicalTVF);
	...
	else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenColumns),
										   element_local_name))
	{
		...
		cold_descr_parse_handler->startElement(
