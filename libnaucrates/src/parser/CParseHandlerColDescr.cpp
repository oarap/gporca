...
//
//---------------------------------------------------------------------------
CParseHandlerColDescr::CParseHandlerColDescr(IMemoryPool *memory_pool,
...
{
	if (0 ==
		XMLString::compareString(element_local_name, CDXLTokens::XmlstrToken(EdxltokenColumns)))
	{
		// start of the columns block
		GPOS_ASSERT(NULL == m_current_column_descr);
	}
	else if (0 ==
			 XMLString::compareString(element_local_name, CDXLTokens::XmlstrToken(EdxltokenColumn)))
	{
		// start of a new column descriptor
		m_current_column_descr =
			CDXLOperatorFactory::MakeColumnDescr(m_parse_handler_mgr->GetDXLMemoryManager(), attrs);
	}
	else
	{
