...
//
//---------------------------------------------------------------------------
CParseHandlerPhysicalSplit::CParseHandlerPhysicalSplit(IMemoryPool *memory_pool,
...
{
	if (0 != XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPhysicalSplit),
									  element_local_name))
	{
...
)
{
	...
	CDXLPhysicalSplit *dxl_op = GPOS_NEW(m_memory_pool) CDXLPhysicalSplit(m_memory_pool,
																		  m_deletion_colid_array,
																		  m_insert_colid_array,
																		  m_action_colid,
																		  m_ctid_colid,
