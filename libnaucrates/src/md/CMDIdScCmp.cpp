...
//
//---------------------------------------------------------------------------
CMDIdScCmp::CMDIdScCmp(CMDIdGPDB *left_mdid, CMDIdGPDB *right_mdid, IMDType::ECmpType cmp_type)
...
CMDIdScCmp::Serialize()
{
	...
	m_str.AppendFormat(GPOS_WSZ_LIT("%d.%d.%d.%d;%d.%d.%d;%d"),
					   MdidType(),
					   m_mdid_left->OidObjectId(),
					   m_mdid_left->VersionMajor(),
					   m_mdid_left->VersionMinor(),
					   m_mdid_right->OidObjectId(),
...
CMDIdScCmp::HashValue() const
{
	return gpos::CombineHashes(
		MdidType(), gpos::CombineHashes(m_mdid_left->HashValue(), m_mdid_right->HashValue()));
}
...
//---------------------------------------------------------------------------
void
CMDIdScCmp::Serialize(CXMLSerializer *xml_serializer, const CWStringConst *attribute_str) const
{
