...
//---------------------------------------------------------------------------
BOOL
CMDTypeGenericGPDB::HasByte2IntMapping(const IMDId *mdid)
{
	return mdid->Equals(&CMDIdGPDB::m_mdid_bpchar) || mdid->Equals(&CMDIdGPDB::m_mdid_varchar) ||
		   mdid->Equals(&CMDIdGPDB::m_mdid_text) || mdid->Equals(&CMDIdGPDB::m_mdid_cash);
}
