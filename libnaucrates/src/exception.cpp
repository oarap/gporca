...
gpdxl::EresExceptionInit(IMemoryPool *memory_pool)
{
	...
	CMessage rgmsg[ExmiDXLSentinel] = {
		...
		CMessage(CException(gpdxl::ExmaDXL, gpdxl::ExmiDXLValidationError),
				 CException::ExsevError,
				 GPOS_WSZ_WSZLEN("XML validation exception: XML document does not conform to the "
								 "DXL schema. Details: %s"),
				 1,  // exception details
		...
		CMessage(CException(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLError),
				 CException::ExsevError,
				 GPOS_WSZ_WSZLEN("Query-to-DXL Translation: %ls entry found due to incorrect "
								 "normalization of query"),
				 1,  // entry name
				 GPOS_WSZ_WSZLEN("Query-to-DXL Translation: Entry not found due to incorrect "
								 "normalization of query")),
