...
CDXLCtasStorageOptions::Serialize(CXMLSerializer *xml_serializer) const
{
	...
	{
		xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenTablespace),
									 m_mdname_tablespace->GetMDName());
	}

	xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenOnCommitAction),
								 GetOnCommitActionStr(m_ctas_on_commit_action));

	const ULONG ulOptions =
		(m_ctas_storage_option_array == NULL) ? 0 : m_ctas_storage_option_array->Size();
	for (ULONG ul = 0; ul < ulOptions; ul++)
	{
		...
		xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenCtasOptionType),
		...
		xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenIsNull),
