...
CDXLPhysicalMotion::GetSegIdsCommaSeparatedStr(const IntPtrArray *segment_ids_array) const
{
	...
	for (ULONG idx = 0; idx < num_of_segments; idx++)
	{
		...
		{
			str->AppendFormat(GPOS_WSZ_LIT("%d%ls"),
							  segment_id,
...
//---------------------------------------------------------------------------
void
CDXLPhysicalMotion::SerializeSegmentInfoToDXL(CXMLSerializer *xml_serializer) const
{
	CWStringDynamic *input_segment_ids_str = GetInputSegIdsStr();
	xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenInputSegments),
								 input_segment_ids_str);

	CWStringDynamic *output_segment_ids_str = GetOutputSegIdsStr();
	xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenOutputSegments),
