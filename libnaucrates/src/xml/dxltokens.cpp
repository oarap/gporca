...
CDXLTokens::SStrMapElem *
CDXLTokens::m_pstrmap = NULL;

CDXLTokens::SXMLStrMapElem *
CDXLTokens::m_pxmlszmap = NULL;

IMemoryPool *
CDXLTokens::m_pmp =  NULL;

CDXLMemoryManager *
CDXLTokens::m_pmm = NULL;
	...
	)
{
	...
	m_pmm = GPOS_NEW(m_pmp) CDXLMemoryManager(m_pmp);
	...
	SWszMapElem rgStrMap[] = 
	{
			...
			{EdxltokenPhysicalPartitionSelectorLevels, GPOS_WSZ_LIT("PartitionLevels")},
			...
			{EdxltokenScalarAssertConstraintList, GPOS_WSZ_LIT("AssertConstraintList")},
			...
			{EdxltokenJoinLeftAntiSemiJoinNotIn, GPOS_WSZ_LIT("LeftAntiSemiJoinNotIn")},
			...
			{EdxltokenWindowBoundaryUnboundedPreceding, GPOS_WSZ_LIT("UnboundedPreceding")},
			{EdxltokenWindowBoundaryBoundedPreceding, GPOS_WSZ_LIT("BoundedPreceding")},
			{EdxltokenWindowBoundaryCurrentRow, GPOS_WSZ_LIT("CurrentRow")},
			{EdxltokenWindowBoundaryUnboundedFollowing, GPOS_WSZ_LIT("UnboundedFollowing")},
			{EdxltokenWindowBoundaryBoundedFollowing, GPOS_WSZ_LIT("BoundedFollowing")},
			{EdxltokenWindowBoundaryDelayedBoundedPreceding, GPOS_WSZ_LIT("DelayedBoundedPreceding")},
			{EdxltokenWindowBoundaryDelayedBoundedFollowing, GPOS_WSZ_LIT("DelayedBoundedFollowing")},
			...
			{EdxltokenXMLDocHeader, GPOS_WSZ_LIT("<?xml version=\"1.0\" encoding=\"UTF-8\"?>")},
			{EdxltokenNamespaceAttr, GPOS_WSZ_LIT("xmlns")},
			{EdxltokenNamespacePrefix, GPOS_WSZ_LIT("dxl")},
			{EdxltokenNamespaceURI, GPOS_WSZ_LIT("http://greenplum.com/dxl/2010/12/")},
			...
			{EdxltokenGPDBAggIntermediateResultTypeId, GPOS_WSZ_LIT("IntermediateResultType")},
	...
	m_pxmlszmap = GPOS_NEW_ARRAY(m_pmp, SXMLStrMapElem, EdxltokenSentinel);
	
	for (ULONG ul = 0; ul < GPOS_ARRAY_SIZE(rgStrMap); ul++)
	{
		SWszMapElem mapelem = rgStrMap[ul];
		
		m_pstrmap[mapelem.m_edxlt].m_pstr = GPOS_NEW(m_pmp) CWStringConst(m_pmp, mapelem.m_wsz);
...
{
	ULONG ulLen = GPOS_WSZ_LENGTH(wsz);
	CHAR *sz = GPOS_NEW_ARRAY(m_pmp, CHAR, 1 + ulLen);
