...
namespace gpnaucrates
{
	...
	public:
		...
		void AddNotExcludedHistograms(IMemoryPool *memory_pool,
	...
	private:
		...
		static void AddHistogramsWithRemap(IMemoryPool *memory_pool,
		...
		static void AddWidthInfoWithRemap(IMemoryPool *memory_pool,
	...
	public:
		// ctor
		CStatistics(IMemoryPool *memory_pool,
		...
		virtual UlongDoubleHashMap *CopyWidths(IMemoryPool *memory_pool) const;

		virtual void CopyWidthsInto(IMemoryPool *memory_pool,
									UlongDoubleHashMap *col_id_width_mapping) const;

		virtual UlongHistogramHashMap *CopyHistograms(IMemoryPool *memory_pool) const;
		...
		virtual CDouble Width(IMemoryPool *memory_pool, CColRefSet *colrefs) const;
		...
		virtual CStatistics *CalcInnerJoinStats(IMemoryPool *memory_pool,
												const IStatistics *other_stats,
												StatsPredJoinArray *join_preds_stats) const;

		// LOJ with another stats structure
		virtual CStatistics *CalcLOJoinStats(IMemoryPool *memory_pool,
											 const IStatistics *other_stats,
											 StatsPredJoinArray *join_preds_stats) const;
		...
		virtual CStatistics *CalcLSJoinStats(IMemoryPool *memory_pool,
											 const IStatistics *inner_side_stats,
											 StatsPredJoinArray *join_preds_stats) const;

		// return required props associated with stats object
		virtual CReqdPropRelational *GetReqdRelationalProps(IMemoryPool *memory_pool) const;

		// append given stats to current object
		virtual void AppendStats(IMemoryPool *memory_pool, IStatistics *stats);
		...
		// copy stats
		virtual IStatistics *CopyStats(IMemoryPool *memory_pool) const;

		// return a copy of this stats object scaled by a given factor
		virtual IStatistics *ScaleStats(IMemoryPool *memory_pool, CDouble factor) const;
		...
		virtual CColRefSet *GetColRefSet(IMemoryPool *memory_pool) const;

		// generate the DXL representation of the statistics object
		virtual CDXLStatsDerivedRelation *GetDxlStatsDrvdRelation(IMemoryPool *memory_pool,
		...
		virtual ULongPtrArray *GetColIdsWithStats(IMemoryPool *memory_pool) const;
		...
		static CStatistics *
		MakeEmptyStats(IMemoryPool *memory_pool)
		{
		...
		static CStatistics *MakeDummyStats(IMemoryPool *memory_pool,
		...
		static void CreateAndInsertUpperBoundNDVs(IMemoryPool *memory_pool,
												  CStatistics *stats,
												  ULongPtrArray *col_ids,
												  CDouble rows);
