...
namespace gpnaucrates
{
	...
	class CBucket: public IBucket
	{
		...
		public:
			...
			CBucket *MakeBucketScaleUpper(IMemoryPool *memory_pool, CPoint *bucket_upper_bound, BOOL include_upper) const;

			// scale down version of bucket adjusting lower boundary
			CBucket *MakeBucketScaleLower(IMemoryPool *memory_pool, CPoint *bucket_lower_bound, BOOL include_lower) const;
			...
			CBucket *MakeBucketIntersect(IMemoryPool *memory_pool, CBucket *bucket, CDouble *result_freq_intersect1, CDouble *result_freq_intersect2) const;

			// Remove a bucket range. This produces lower and upper split
			void Difference(IMemoryPool *memory_pool, CBucket *bucket_other, CBucket **result_bucket_lower, CBucket **result_bucket_upper);
