...
namespace gpnaucrates
{
	...
	class CStatisticsUtils
	{
		...
		private:
			...
			static
			BucketArray *MergeMcvHistBucket
						(
			...
			static
			BucketArray *SplitHistBucketGivenMcvBuckets
						(
						IMemoryPool *memory_pool,
			...
			static
			BOOL CappedGrpColExists(const CStatistics *stats, const ULongPtrArray *grouping_columns);

			// return the maximum NDV given an array of grouping columns
			static
			CDouble MaxNdv(const CStatistics *stats,const ULongPtrArray *grouping_columns);
		...
		public:
			...
			static
			inline INT GetMcvPairCmpFunc(const void *val1, const void *val2);
			...
			static
			UlongHistogramHashMap *CreateHistHashMapAfterMergingDisjPreds
						(
						IMemoryPool *memory_pool,
						CBitSet *non_updatable_cols,
			...
			static
			UlongHistogramHashMap *CopyHistHashMap(IMemoryPool *memory_pool, UlongHistogramHashMap *col_histogram_mapping);

			// return the column identifier of the filter if the predicate is
			// on a single column else	return gpos::ulong_max
			static
			ULONG GetColId(const StatsPredPtrArry *stats_preds_array);
			...
			static
			IDatum *DatumNull(const CColRef *colref);

#ifdef GPOS_DEBUG
			// helper method to print the hash map of histograms
			static
			void PrintHistogramMap(IOstream &os, UlongHistogramHashMap *col_histogram_mapping);
#endif // GPOS_DEBUG

			// derive statistics of dynamic scan based on part-selector stats in the given map
			static
			IStatistics *DeriveStatsForDynamicScan(IMemoryPool *memory_pool, CExpressionHandle &expr_handle, ULONG part_idx_id, CPartFilterMap *part_filter_map);

			// derive statistics of (dynamic) index-get
			static
			IStatistics *DeriveStatsForIndexGet(IMemoryPool *memory_pool, CExpressionHandle &exprhdl, StatsArray *stats_contexts);

			// derive statistics of bitmap table-get
			static
			IStatistics *DeriveStatsForBitmapTableGet(IMemoryPool *memory_pool, CExpressionHandle &exprhdl, StatsArray *stats_contexts);
			...
			static
			UlongUlongArrayHashMap *GetGrpColIdToUpperBoundNDVIdxMap
							(
							IMemoryPool *memory_pool,
							CStatistics *stats,
			...
			static
			CDouble GetNumDistinct(const BucketArray *histogram_buckets);

			// return the cumulative frequency in the given array of buckets
			static
			CDouble GetFrequency(const BucketArray *histogram_buckets);

			// true if the given operator increases risk of cardinality misestimation
			static
			BOOL IncreasesRisk(CLogical *logical_op);

			// return the default column width
			static
			CDouble DefaultColumnWidth(const IMDType *mdtype);

			// helper method to add width information
			static
			void AddWidthInfo(IMemoryPool *memory_pool, UlongDoubleHashMap *src_width, UlongDoubleHashMap *dest_width);
	...
	// comparison function for sorting MCVs
	INT CStatisticsUtils::GetMcvPairCmpFunc
		(
		const void *val1,
		const void *val2
		)
