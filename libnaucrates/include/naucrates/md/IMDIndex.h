...
namespace gpmd
{
	...
	class IMDIndex : public IMDCacheObject
	{
		public:
			...
			virtual
			BOOL IsClustered() const = 0;
			
			// index type
			virtual
			EmdindexType IndexType() const = 0;
			...
			virtual
			ULONG KeyAt(ULONG pos) const = 0;

			// return the position of the key column
			virtual
			ULONG GetKeyPos(ULONG pos) const = 0;

			// number of included columns
			virtual
			ULONG IncludedCols() const = 0;

			// return the n-th included column
			virtual
			ULONG IncludedColAt(ULONG pos) const = 0;

			// return the position of the included column
			virtual
			ULONG GetIncludedColPos(ULONG column) const = 0;
			...
			virtual
			IMDId *GetIndexRetItemTypeMdid() const = 0;

			// check if given scalar comparison can be used with the index key 
			// at the specified position
			virtual 
			BOOL IsCompatible(const IMDScalarOp *md_scalar_op, ULONG key_pos) const = 0;
			
			// index type as a string m_bytearray_value
			static
			const CWStringConst *GetDXLStr(EmdindexType index_type);
	};
}
