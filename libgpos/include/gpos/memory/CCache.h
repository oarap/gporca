...
namespace gpos
{
	...
	private:
		...
		typedef CSyncHashtable<CCacheHashTableEntry, K, CSpinlockCache> CCacheHashtable;
		typedef CSyncHashtableAccessByKey<CCacheHashTableEntry, K, CSpinlockCache>
			CCacheHashtableAccessor;
		typedef CSyncHashtableIter<CCacheHashTableEntry, K, CSpinlockCache> CCacheHashtableIter;
			...
			if (CompareSwap(&m_eviction_lock, 0, 1))
			{
				if (m_cache_size > m_cache_quota)
				{
					double to_free = static_cast<double>(static_cast<double>(m_cache_size) -
														 static_cast<double>(m_cache_quota) *
															 (1.0 - m_eviction_factor));
