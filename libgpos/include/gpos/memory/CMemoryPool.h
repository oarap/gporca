...
#define GPOS_MEM_ALIGNED_SIZE(x) \
	(GPOS_MEM_ARCH * (((x) / GPOS_MEM_ARCH) + ((x) % GPOS_MEM_ARCH ? 1 : 0)))
...
namespace gpos
{
	...
	public:
		...
		static void FreeAlloc(void *ptr, EAllocationType eat);
		...
		virtual void
		UpdateStatistics(CMemoryPoolStatistics &)
		{
			GPOS_ASSERT(!"not supported");
		}
