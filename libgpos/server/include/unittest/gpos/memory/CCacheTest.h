...
namespace gpos
{
	...
	private:
		...
		struct SSimpleObject : public CRefCount
		{
			...
			SSimpleObject(ULONG ulKey, ULONG ulVal) : m_ulKey(ulKey), m_ulValue(ulVal)
			{
			}
		...
		static ULONG ULFillCacheWithoutEviction(CCache<SSimpleObject *, ULONG *> *pCache,
												ULONG ulKeyStart);

		// checks if after eviction we have more entries from newer generation than the older generation
		static void CheckGenerationSanityAfterEviction(CCache<SSimpleObject *, ULONG *> *pCache,
													   ULLONG ullOneElemSize,
		...
		private:
			...
			struct SDeepObjectEntry
			{
				...
				// ctor
				SDeepObjectEntry(ULONG ulKey, ULONG ulVal) : m_ulKey(ulKey), m_ulValue(ulVal)
				{
				}
