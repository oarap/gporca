...
COstreamStringTest::EresUnittest()
{
	CUnittest rgut[] = {
		...
		GPOS_UNITTEST_FUNC_THROW(
			COstreamStringTest::EresUnittest_OOM, CException::ExmaSystem, CException::ExmiOOM),
...
COstreamStringTest::EresUnittest_OOM()
{
	// create memory pool
	CAutoMemoryPool amp(
		CAutoMemoryPool::ElcExc, CMemoryPoolManager::EatTracker, false /*fThreadSafe*/, 1024);
	IMemoryPool *memory_pool = amp.Pmp();
