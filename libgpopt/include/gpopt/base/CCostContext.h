...
namespace gpopt
{
	...
	class CCostContext : public CRefCount
	{
		...
		private:
			...
			static
			void BreakCostTiesForJoinPlans(const CCostContext *pccFst, const CCostContext *pccSnd, CONST_COSTCTXT_PTR *ppccPrefered, BOOL *pfTiesResolved);
		...
		public:
			...
			CGroupExpression *PgexprForStats() const
			{
			...
			// compute cost
			CCost CostCompute(IMemoryPool *mp, CostArray *pdrgpcostChildren);
