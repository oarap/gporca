...
namespace gpopt
{
	...
	class CJobGroupExpressionOptimization : public CJobGroupExpression
	{
		...
		private:
			...
			// initialize action
			static
			EEvent EevtInitialize(CSchedulerContext *psc, CJob *pj);
			...
			static
			EEvent EevtAddEnforcers(CSchedulerContext *psc, CJob *pj);

			// optimize group expression action
			static
			EEvent EevtOptimizeSelf(CSchedulerContext *psc, CJob *pj);
		...
		protected:
			...
			virtual
			void ScheduleApplicableTransformations
				(
				CSchedulerContext * // psc
				)
			{
		...
		public:
			...
			// initialize job
			void Init(CGroupExpression *pgexpr, COptimizationContext *poc, ULONG ulOptReq, CReqdPropPlan *prppCTEProducer = NULL);
			...
			// conversion function
			static
			CJobGroupExpressionOptimization *PjConvert
				(
				CJob *pj
				)
			{
