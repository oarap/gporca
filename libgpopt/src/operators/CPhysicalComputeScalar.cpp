	...
	)
{
	...
	CColRefSet *pcrsChildReqd = PcrsChildReqd(memory_pool, exprhdl, pcrs, child_index, 1 /*ulScalarIndex*/);
	pcrs->Release();
	...
	const
{
	...
	{
		return GPOS_NEW(memory_pool) CDistributionSpecStrictSingleton(CDistributionSpecSingleton::EstMaster);
	}
