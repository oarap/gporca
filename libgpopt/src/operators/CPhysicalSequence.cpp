	...
	m_pcrsEmpty(NULL)
{
	...
	m_pcrsEmpty = GPOS_NEW(memory_pool) CColRefSet(memory_pool);
}
	...
	const
{
	...
	CCTEReq *pcterUnresolved = pcter->PcterUnresolvedSequence(memory_pool, pcmCombined, pdrgpdpCtxt);
	pcmCombined->Release();
	...
	const
{
	...
	if (0 == ulOptReq)
	{
			...
			CDistributionSpec::EdtStrictSingleton == pdsRequired->Edt())
		{
			...
			return GPOS_NEW(memory_pool) CDistributionSpecSingleton(pdss->Est());
		}
	...
	const
{
	...
	return GPOS_NEW(m_memory_pool) CRewindabilitySpec(CRewindabilitySpec::ErtNone);
}
	...
	const
{
	...
	return GPOS_NEW(m_memory_pool) CRewindabilitySpec(CRewindabilitySpec::ErtNone /*ert*/);
}
