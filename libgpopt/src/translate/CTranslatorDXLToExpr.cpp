	...
	)
{
	...
	*ppexprChild = PexprLogical(pdxlnChild);
	...
	for (ULONG ulColPos = 0; ulColPos < ulInputCols; ulColPos++)
	{
		...
		OID oidCurrentCollation = pcr->OidCollation(); //pdxlcdOutput->OidCollation();
		OID oidOutCollation = *(*pdxlop->PdrgpulColCollations())[ulColPos];
	...
	)
{
	...
	IMDId *pmdid = pdxlop->PmdidType();
	pmdid->AddRef();
	CScalarSwitch *pop = GPOS_NEW(m_pmp) CScalarSwitch(m_pmp, pmdid, pdxlop->OidCollation());
	...
	)
{
	...
	IMDId *pmdid = pdxlop->PmdidType();
	pmdid->AddRef();
	CScalarCaseTest *pop = GPOS_NEW(m_pmp) CScalarCaseTest(m_pmp, pmdid, pdxlop->OidCollation());
	...
	)
{
	...
	else
	{
		...
		pexpr= GPOS_NEW(m_pmp) CExpression
									(
									m_pmp,
									GPOS_NEW(m_pmp) CScalarCast(m_pmp, pmdidType, pmdidFunc, fRelabel, oidResultCollation, oidInputCollation),
									pexprChild
									);
	}
