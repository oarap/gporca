...
CStatisticsTest::EresUnittest_UnionAll()
{
	...
	for (ULONG i = 0; i < ulTestCases; i++)
	{
		...
		CStatistics *pstatsOutput = CUnionAllStatsProcessor::CreateStatsForUnionAll(mp, pstats1, pstats2, pdrgpulColIdOutput, pdrgpulColIdInput1, pdrgpulColIdInput2);
...
CStatisticsTest::EresUnittest_GbAggWithRepeatedGbCols()
{
	...
	CExpression *pexprGbAgg2 =
			CUtils::PexprLogicalGbAggGlobal(mp, pdrgpcr2, pexpr, GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
	...
	CExpression *pexprGbAgg3 =
			CUtils::PexprLogicalGbAggGlobal(mp, pdrgpcr3, pexprPrj, GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp)));
...
CStatisticsTest::EresUnittest_CStatisticsBasic()
{
	...
	CStatistics *pstats4 = CGroupByStatsProcessor::CalcGroupByStats(mp, stats, GCs, aggs, NULL /*keys*/);

	GPOS_TRACE(GPOS_WSZ_LIT("pstats4 = stats group by"));
	CCardinalityTestUtils::PrintStats(mp, pstats4);

	// LASJ stats
	CStatistics *pstats5 = stats->CalcLASJoinStats(mp, pstats2, join_preds_stats, true /* DoIgnoreLASJHistComputation */);
	...
	CStatistics *pstats7 = CLimitStatsProcessor::CalcLimitStats(mp, stats, CDouble(4.0));
	...
	)
{
	...
	CWStringDynamic *pstrLowerNumeric = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("AAAACgAAAQABAA=="));
	CWStringDynamic *pstrUpperNumeric = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("AAAACgAAAQACAA=="));

	StatsFilterNumeric(mp, 3, pstrLowerNumeric, pstrUpperNumeric, CDouble(1.0), CDouble(2.0), pdrgpstatspred);

	GPOS_DELETE(pstrLowerNumeric);
	GPOS_DELETE(pstrUpperNumeric);

	// create a date filter column 4: ['01-01-2012', '01-21-2012')::date
	CWStringDynamic *pstrLowerDate = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("HxEAAA=="));
	CWStringDynamic *pstrUpperDate = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("LREAAA=="));
	...
	StatsFilterGeneric(mp, 4, GPDB_DATE, pstrLowerDate, pstrUpperDate, lLowerDate, lUpperDate, pdrgpstatspred);
	...
	CWStringDynamic *pstrLowerTS = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("ACcI7mpYAQA="));
	CWStringDynamic *pstrUpperTS = GPOS_NEW(mp) CWStringDynamic(mp, GPOS_WSZ_LIT("AAg5THNYAQA="));
	LINT lLowerTS = LINT(INT64_C(378691260000000)); // microseconds
	LINT lUpperTS = LINT(INT64_C(378727200000000)); // microseconds

	StatsFilterGeneric(mp, 5, GPDB_TIMESTAMP, pstrLowerTS, pstrUpperTS, lLowerTS, lUpperTS, pdrgpstatspred);
