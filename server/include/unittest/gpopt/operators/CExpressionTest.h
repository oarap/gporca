...
namespace gpopt
{
	...
	{			
		private:
			static
			CReqdPropPlan *PrppCreateRequiredProperties(IMemoryPool *memory_pool, CColRefSet *pcrs);

			static
			CExpression *PexprCreateGbyWithColumnFormat(IMemoryPool *memory_pool, const WCHAR *wszColNameFormat);

			// helper for testing required column computation
			static
			GPOS_RESULT EresComputeReqdCols(const CHAR *szFilePath);
			...
			static
			CExpression *PexprComplexJoinTree(IMemoryPool *memory_pool);
		...
		public:
			...
			static
			GPOS_RESULT EresUnittest_FValidPlan();

			static
			GPOS_RESULT EresUnittest_FValidPlan_InvalidOrder();

			static
			GPOS_RESULT EresUnittest_FValidPlan_InvalidDistribution();

			static
			GPOS_RESULT EresUnittest_FValidPlan_InvalidRewindability();

			static
			GPOS_RESULT EresUnittest_FValidPlan_InvalidCTEs();

			static
			GPOS_RESULT EresUnittest_FValidPlanError();

			// test for required columns computation
			static
			GPOS_RESULT EresUnittest_ReqdCols();

			// negative test for invalid SetOp expression
			static
			GPOS_RESULT EresUnittest_InvalidSetOp();
